...
assign it back to coutry

```{r}
cluster_assignments <- clustering_result@cluster

## Extract country names from the list_matrices_per_country or bmi_wide if names are preserved
country_names <- bmi_wide$country %>% unique()

# Ensure the length matches to avoid potential mismatches
if(length(country_names) == length(cluster_assignments)) {
  # Create a DataFrame mapping countries to their clusters
