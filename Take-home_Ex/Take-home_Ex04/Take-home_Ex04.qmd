...
ggplot(subset(bmi, year == "2015"), aes(x = country, y = bmi_usd_price)) + 
  geom_bar(stat = "identity") + 
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) + 
  labs(title = "BMI Price Comparison by Country for 2015", y = "BMI USD Price", x = "Country")

```

```{r}
# Assuming the benchmark is set by the top 5 GDP per capita countries
benchmark_countries <- bmi[order(-bmi$gdp_per_capita),][1:5,]
# Assuming you want to compare bmi_usd_price
ggplot(bmi, aes(x = country, y = bmi_usd_price, fill = country %in% benchmark_countries$country)) + 
  geom_bar(stat = "identity") + 
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) + 
  labs(title = "BMI USD Price Comparison with Benchmark Countries", y = "BMI USD Price", x = "Country")

```

### **Distribution Analysis**

Investigate the distribution of key variables like **`bmi_usd_price`**, **`GDP`**, and **`inflation`** across different countries. This involves using histograms, box plots, or density plots to understand the spread and skewness of the data.

```{r}
# Histogram for GDP
ggplot(bmi, aes(x = GDP)) + 
  geom_histogram(binwidth = 5000, fill = "blue", color = "black") +
  labs(title = "GDP Distribution Across Countries", x = "GDP", y = "Frequency")

# Boxplot for bmi_usd_price
ggplot(bmi, aes(y = bmi_usd_price)) + 
  geom_boxplot(fill = "lightblue") +
  labs(title = "BMI USD Price Distribution Across Countries", y = "BMI USD Price")

```

### **Outlier Detection**

Identify any outliers in your dataset for variables of interest. Outliers can distort statistical analysis and may indicate data entry errors, or genuinely interesting observations that merit further investigation.

```{r}
ggplot(bmi, aes(y = bmi_usd_price)) + 
  geom_boxplot() +
  labs(title = "Outliers in BMI USD Price")
